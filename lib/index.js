...
var debug = process.execArgv[0] && process.execArgv[0].indexOf('--debug') !== -1;
if(debug) {   
...
function fork(modulePath, args, options) {
    ...
    var p = child_process.fork(modulePath, args, options);

    p.on('error', deferred.reject);
    p.on('close', function (code) {
        if (code !== 0) {
            var commandStr = modulePath + (args.length ? (' ' + args.join(' ')) : '');
            deferred.reject({
                code: code,
                message: '"' + commandStr + '" failed with code ' + code
            });
